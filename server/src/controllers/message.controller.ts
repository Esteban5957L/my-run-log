...
export async function getMessages(req: Request, res: Response) {
  try {
    ...
    const messagesWithDefaults = messages.map(msg => ({
      ...
      attachmentType: (msg as any).attachmentType || null,
    ...
    res.json({ messages: messagesWithDefaults.reverse() });
  } catch (error) {
...
export async function sendMessage(req: Request, res: Response) {
  try {
    ...
    // Crear mensaje solo con campos básicos (los nuevos campos se agregarán cuando se aplique la migración)
...
// Agregar reacción a un mensaje (deshabilitado hasta que se aplique la migración)
export async function addReaction(req: Request, res: Response) {
  // Las reacciones requieren una migración de base de datos
  // Por ahora retornamos un mensaje indicando que no está disponible
  return res.status(501).json({ error: 'Las reacciones estarán disponibles próximamente' });
}
